...
class Spreadsheet
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        $this->calculationEngine = new \PhpOffice\PhpSpreadsheet\Calculation\Calculation($this);
        ...
        $this->properties = new \PhpOffice\PhpSpreadsheet\Document\Properties();

        // Create document security
        $this->security = new \PhpOffice\PhpSpreadsheet\Document\Security();

        // Set named ranges
        $this->namedRanges = array();

        // Create the cellXf supervisor
        $this->cellXfSupervisor = new \PhpOffice\PhpSpreadsheet\Style\Style(\true);
     ...
     * Set properties
     *
     * @param \PhpOffice\PhpSpreadsheet\Document\Properties    $pValue
     */
     ...
     * Set security
     *
     * @param \PhpOffice\PhpSpreadsheet\Document\Security    $pValue
     */
    ...
    {
        $newSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($this);
     ...
     * @throws \PhpOffice\PhpSpreadsheet\Exception
     */
    public function addSheet(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet, $iSheetIndex = \null)
    {
    ...
    {
        if (($worksheet = $this->getSheetByName($pValue)) instanceof \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet) {
     ...
     * Add external sheet
     *
     * @param  \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet External sheet to add
     * @param  int|null $iSheetIndex Index where sheet should go (0,1,..., or null for last)
     * @throws \PhpOffice\PhpSpreadsheet\Exception
     * @return \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet
     */
    public function addExternalSheet(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet, $iSheetIndex = \null)
    {
        if ($this->sheetNameExists($pSheet->getTitle())) {
            throw new \PhpOffice\PhpSpreadsheet\Exception("Workbook already contains a worksheet named '{$pSheet->getTitle()}'. Rename the external sheet first.");
        }
        ...
        // update the cellXfs
        foreach ($pSheet->getCoordinates(\false) as $cellID) {
     ...
     */
    public function getNamedRange($namedRange, \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet = \null)
    {
        $returnValue = \null;
        ...
        if ($namedRange != '' && ($namedRange !== \null)) {
            ...
            if (($pSheet !== \null) && isset($this->namedRanges[$pSheet->getTitle() . '!' . $namedRange])) {
     ...
     */
    public function removeNamedRange($namedRange, \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet = \null)
    {
        if ($pSheet === \null) {
            if (isset($this->namedRanges[$namedRange])) {
    ...
    public function removeCellXfByIndex($pIndex = 0)
    {
        if ($pIndex > \count($this->cellXfCollection) - 1) {
            ...
            foreach ($this->workSheetCollection as $worksheet) {
                foreach ($worksheet->getCoordinates(\false) as $cellID) {
                    $cell = $worksheet->getCell($cellID, true);
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Style $pStyle
     */
    ...
    public function garbageCollect()
    {
        ...
        foreach ($this->getWorksheetIterator() as $sheet) {
            // from cells
            foreach ($sheet->getCoordinates(\false) as $cellID) {
                $cell = $sheet->getCell($cellID, true);
