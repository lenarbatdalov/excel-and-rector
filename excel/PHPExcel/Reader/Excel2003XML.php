...
//                        echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
                        $objPHPExcel->getActiveSheet()->getColumnDimension($columnID, true)->setWidth($columnWidth / 5.4);
                    }
                    ++$columnID;
                }
            }
...
//
                            $objPHPExcel->getActiveSheet()->getCell($columnID.$rowID, true)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue), $type);
                            if ($hasCalculatedValue) {
//                                echo 'Formula result is '.$cellValue.'<br />';
                                $objPHPExcel->getActiveSheet()->getCell($columnID.$rowID, true)->setCalculatedValue($cellValue);
                            }
...
//                                echo '<br />';
                                if (!$objPHPExcel->getActiveSheet()->cellExists($columnID.$rowID)) {
                                    $objPHPExcel->getActiveSheet()->getCell($columnID.$rowID, true)->setValue(\null);
                                }
                                $objPHPExcel->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->styles[$style], true);
                            }
                        }
                        ++$columnID;
...
//                            echo '<b>Setting row height to '.$rowHeight.'</b><br />';
                            $objPHPExcel->getActiveSheet()->getRowDimension($rowID, true)->setRowHeight($rowHeight);
                        }
                    }
