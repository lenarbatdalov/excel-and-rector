 ...
 */
class Xlsx extends \PhpOffice\PhpSpreadsheet\Reader\BaseReader implements \PhpOffice\PhpSpreadsheet\Reader\IReader
{
    /**
     ...
     *
     * @var \PhpOffice\PhpSpreadsheet\Reader\Xlsx\Theme
     */
...
//        }
        ...
        if ($zip->open($pFilename) === \true) {
            ...
            $rels = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
            if ($rels !== \false) {
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $rels = \simplexml_load_string(
            $this->securityScan($this->getFromZipArchive($zip, "_rels/.rels"), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions())
        ); //~ http://schemas.openxmlformats.org/package/2006/relationships");
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    $xmlWorkbook = \simplexml_load_string(
                        $this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}"), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions())
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        $rels = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
        foreach ($rels->Relationship as $rel) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                $dir = \dirname($rel["Target"]);
                $relsWorkbook = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . \basename($rel["Target"]) . ".rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/package/2006/relationships");
                ...
                $xmlWorkbook = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                if ($xmlWorkbook->sheets) {
                    $dir = \dirname($rel["Target"]);
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        $tmpInfo = array(
                        ...
                        $xml = new \XMLReader();
                        $res = $xml->xml($this->securityScanFile('zip://'.\PhpOffice\PhpSpreadsheet\Shared\File::realpath($pFilename).'#'."$dir/$fileWorksheet"), \null, \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                        $xml->setParserProperty(2, \true);
...
//                echo 'Formula is ', $sharedFormulas[$instance]['formula'], PHP_EOL;
                $master = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($sharedFormulas[$instance]['master']);
                $current = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($r);

                $difference = array(0, 0);
                $difference[0] = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($current[0]) - \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($master[0]);
...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    public function load($pFilename)
    {
        ...
        $wbRels = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
        foreach ($wbRels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    $xmlTheme = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/{$rel['Target']}")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                    if (\is_object($xmlTheme)) {
                        ...
                        self::$theme = new \PhpOffice\PhpSpreadsheet\Reader\Xlsx\Theme($themeName, $colourSchemeName, $themeColours);
                    }
                    break;
            }
        }
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
                    $xmlCore = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $xmlStrings = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                    ...
                    $xmlStyles = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                    ...
                    if (!$this->readDataOnly && $xmlStyles) {
                        foreach ($xmlStyles->cellXfs->xf as $xf) {
                            ...
                            if ($xf["numFmtId"]) {
                                ...
                                if ((int)$xf["numFmtId"] < 164 && \PhpOffice\PhpSpreadsheet\Style\NumberFormat::builtInFormatCode((int)$xf["numFmtId"]) !== '') {
                            ...
                            $objStyle = new \PhpOffice\PhpSpreadsheet\Style\Style;
                    ...
                    if (!$this->readDataOnly && $xmlStyles) {
                        ...
                        if ($xmlStyles->dxfs) {
                            foreach ($xmlStyles->dxfs->dxf as $dxf) {
                                $style = new \PhpOffice\PhpSpreadsheet\Style\Style(\false, \true);
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            // Load sheet
                            $docSheet = $excel->createSheet(null);
                            ...
                            $fileWorksheet = $worksheets[(string) self::getArrayItem($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
                            $xmlSheet = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$fileWorksheet")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                            ...
                            if (isset($xmlSheet->sheetPr) && isset($xmlSheet->sheetPr->outlinePr)) {
                                if (isset($xmlSheet->sheetPr->outlinePr['summaryRight']) &&
                                    ...
                                    $docSheet->setShowSummaryRight(\true);
                                }
                                ...
                                if (isset($xmlSheet->sheetPr->outlinePr['summaryBelow']) &&
                                    ...
                                    $docSheet->setShowSummaryBelow(\true);
                                }
                            }
                            ...
                            if (isset($xmlSheet->sheetPr) && isset($xmlSheet->sheetPr->pageSetUpPr)) {
                                if (isset($xmlSheet->sheetPr->pageSetUpPr['fitToPage']) &&
                                    ...
                                    $docSheet->getPageSetup()->setFitToPage(\true);
                                }
                            }
                            ...
                            if (isset($xmlSheet->sheetFormatPr)) {
                                ...
                                if (isset($xmlSheet->sheetFormatPr['zeroHeight']) &&
                                    ...
                                    $docSheet->getDefaultRowDimension()->setZeroHeight(\true);
                                }
                            }
                                ...
                                foreach ($xmlSheet->cols->col as $col) {
                                    for ($i = \intval($col["min"]) - 1; $i < \intval($col["max"]); ++$i) {
                                        if ($col["style"] && !$this->readDataOnly) {
                                            $docSheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i), true)->setXfIndex(\intval($col["style"]));
                                        }
                                        ...
                                        // echo PHPExcel_Cell::stringFromColumnIndex($i), ': HIDDEN COLUMN',PHP_EOL;
                                            $docSheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i), true)->setVisible(\false);
                                        }
                                        if (self::boolean($col["collapsed"])) {
                                            $docSheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i), true)->setCollapsed(\true);
                                        }
                                        if ($col["outlineLevel"] > 0) {
                                            $docSheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i), true)->setOutlineLevel(\intval($col["outlineLevel"]));
                                        }
                                        $docSheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i), true)->setWidth(\floatval($col["width"]));
                            ...
                            if ($xmlSheet && $xmlSheet->sheetData && $xmlSheet->sheetData->row) {
                                foreach ($xmlSheet->sheetData->row as $row) {
                                    if ($row["ht"] && !$this->readDataOnly) {
                                        $docSheet->getRowDimension(\intval($row["r"]), true)->setRowHeight(\floatval($row["ht"]));
                                    }
                                    if (self::boolean($row["hidden"]) && !$this->readDataOnly) {
                                        $docSheet->getRowDimension(\intval($row["r"]), true)->setVisible(\false);
                                    }
                                    if (self::boolean($row["collapsed"])) {
                                        $docSheet->getRowDimension(\intval($row["r"]), true)->setCollapsed(\true);
                                    }
                                    if ($row["outlineLevel"] > 0) {
                                        $docSheet->getRowDimension(\intval($row["r"]), true)->setOutlineLevel(\intval($row["outlineLevel"]));
                                    }
                                    if ($row["s"] && !$this->readDataOnly) {
                                        $docSheet->getRowDimension(\intval($row["r"]), true)->setXfIndex(\intval($row["s"]));
                                    }
                                    ...
                                    foreach ($row->c as $c) {
                                        ...
                                        if ($this->getReadFilter() !== \null) {
                                            $coordinates = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($r);
    ...
    //                                            echo 'Boolean', PHP_EOL;
                                                if (!isset($c->f)) {
                                                    ...
                                                    if (isset($c->f['t'])) {
                                                        $att = array();
                                                        $att = $c->f;
                                                        $docSheet->getCell($r, true)->setFormulaAttributes($att);
                                                    }
...
//                                                echo 'Default', PHP_EOL;
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                                        ...
                                        // Rich text?
                                        if ($value instanceof \PhpOffice\PhpSpreadsheet\RichText\RichText && $this->readDataOnly) {
                                            $value = $value->getPlainText();
                                        }

                                        $cell = $docSheet->getCell($r, true);
                                        // Assign value
                                        if ($cellDataType != '') {
                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->sheetProtection) {
                                $docSheet->getProtection()->setPassword((string) $xmlSheet->sheetProtection["password"], \true);
                                if ($xmlSheet->protectedRanges->protectedRange) {
                                    foreach ($xmlSheet->protectedRanges->protectedRange as $protectedRange) {
                                        $docSheet->protectCells((string) $protectedRange["sqref"], (string) $protectedRange["password"], \true);
                                    }
                                }
                            }
                                    ...
                                    foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                        ...
                                        if ($filterColumn->filters) {
                                            ...
                                            if ((isset($filters["blank"])) && ($filters["blank"] == 1)) {
                                                ...
                                                $column->createRule()->setRule(\null, '', null)->setRuleType(\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_FILTER);
                                            }
                                            //    Standard filters are always an OR join, so no join rule needs to be set
                                            //    Entries can be either filter elements
                                            foreach ($filters->filter as $filterRule) {
                                                //    Operator is undefined, but always treated as EQUAL
                                                $column->createRule()->setRule(\null, (string) $filterRule["val"], null)->setRuleType(\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_FILTER);
                                            }
                            ...
                            if ($xmlSheet && $xmlSheet->headerFooter && !$this->readDataOnly) {
                                ...
                                if (isset($xmlSheet->headerFooter["differentOddEven"]) &&
                                    ...
                                    $docHeaderFooter->setDifferentOddEven(\true);
                                } else {
                                    $docHeaderFooter->setDifferentOddEven(\false);
                                }
                                if (isset($xmlSheet->headerFooter["differentFirst"]) &&
                                    self::boolean((string)$xmlSheet->headerFooter["differentFirst"])) {
                                    $docHeaderFooter->setDifferentFirst(\true);
                                } else {
                                ...
                                if (isset($xmlSheet->headerFooter["scaleWithDoc"]) &&
                                    ...
                                    $docHeaderFooter->setScaleWithDocument(\true);
                                }
                                if (isset($xmlSheet->headerFooter["alignWithMargins"]) &&
                                    !self::boolean((string)$xmlSheet->headerFooter["alignWithMargins"])) {
                                    $docHeaderFooter->setAlignWithMargins(\false);
                                } else {
                                    $docHeaderFooter->setAlignWithMargins(\true);
                                }
                            ...
                            if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->readDataOnly) {
                                foreach ($xmlSheet->rowBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak("A$brk[id]", \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW);
                                    }
                                }
                            }
                            if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->readDataOnly) {
                                foreach ($xmlSheet->colBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $brk["id"]) . "1", \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_COLUMN);
                                    }
                                }
                            }
                            ...
                            if ($xmlSheet && $xmlSheet->dataValidations && !$this->readDataOnly) {
                                foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
                                    ...
                                    foreach ($rangeSet as $range) {
                                        ...
                                        foreach (\PhpOffice\PhpSpreadsheet\Cell\Coordinate::extractAllCellReferencesInRange($stRange) as $reference) {
                                            // Create validation
                                            $docValidation = $docSheet->getCell($reference, true)->getDataValidation();
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(\dirname("$dir/$fileWorksheet") . "/_rels/" . \basename($fileWorksheet) . ".rels")) {
                                    $relsWorksheet = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, \dirname("$dir/$fileWorksheet") . "/_rels/" . \basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                ...
                                if ($xmlSheet && $xmlSheet->hyperlinks) {
                                    foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
                                        ...
                                        foreach (\PhpOffice\PhpSpreadsheet\Cell\Coordinate::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
                                            $cell = $docSheet->getCell($cellReference, true);
                                            if (isset($linkRel['id'])) {
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    ...
                                    $commentsFile = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    $relPath = \PhpOffice\PhpSpreadsheet\Shared\File::realpath(\dirname("$dir/$fileWorksheet") . "/" . $relPath);
                                    $vmlCommentsFile = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                                ...
                                if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->readDataOnly) {
                                    if ($zip->locateName(\dirname("$dir/$fileWorksheet") . "/_rels/" . \basename($fileWorksheet) . ".rels")) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            // Fetch linked images
                                            $relsVML = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, \dirname($vmlRelationship) . '/_rels/' . \basename($vmlRelationship) . '.rels')), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                            $drawings = array();
                                            ...
                                            // Fetch VML document
                                            $vmlDrawing = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $vmlRelationship)), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
                                            ...
                                            foreach ($shapes as $idx => $shape) {
                                                ...
                                                $hfImages[ (string)$shape['id'] ]->setPath("zip://".\PhpOffice\PhpSpreadsheet\Shared\File::realpath($pFilename)."#" . $drawings[(string)$imageData['relid']], \false);
                                                ...
                                                $hfImages[ (string)$shape['id'] ]->setResizeProportional(\true);
                                            }
                            ...
                            if ($zip->locateName(\dirname("$dir/$fileWorksheet") . "/_rels/" . \basename($fileWorksheet) . ".rels")) {
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        $relsDrawing = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, \dirname($fileDrawing) . "/_rels/" . \basename($fileDrawing) . ".rels")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                        $images = array();
                                        ...
                                        $xmlDrawing = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $fileDrawing)), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions())->children("http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing");
                                        ...
                                        if ($xmlDrawing->oneCellAnchor) {
                                            foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing->setPath("zip://".\PhpOffice\PhpSpreadsheet\Shared\File::realpath($pFilename)."#" . $images[(string) self::getArrayItem($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], \false);
                                                    $objDrawing->setCoordinates(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1));
                                                    ...
                                                    $coordinates    = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $oneCellAnchor->from->col) . ($oneCellAnchor->from->row + 1);
                                        ...
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing->setPath("zip://".\PhpOffice\PhpSpreadsheet\Shared\File::realpath($pFilename)."#" . $images[(string) self::getArrayItem($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], \false);
                                                    $objDrawing->setCoordinates(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1));
                                                    ...
                                                    $fromCoordinate = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $twoCellAnchor->from->col) . ($twoCellAnchor->from->row + 1);
                                                    ...
                                                    $toCoordinate   = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex((string) $twoCellAnchor->to->col) . ($twoCellAnchor->to->row + 1);
                                ...
                                if ((string)$definedName['localSheetId'] != '') {
                                    ...
                                    switch ((string)$definedName['name']) {
                                        ...
                                        case '_xlnm.Print_Area':
                                            break;
                                        default:
                                            if ($mapSheetId[(integer) $definedName['localSheetId']] !== \null) {
                                                ...
                                                if (\count($range) == 2) {
                                                    ...
                                                    if ($worksheet = $docSheet->getParent()->getSheetByName($range[0])) {
                                                        ...
                                                        $excel->addNamedRange(new \PhpOffice\PhpSpreadsheet\NamedRange((string)$definedName['name'], $worksheet, $extractedRange, \true, $scope));
                                                    }
                                                }
                                            }
                                            break;
                                    ...
                                    if (\strpos((string)$definedName, '!') !== \false) {
                                        // Extract sheet name
                                        $extractedSheetName = \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::extractSheetTitle((string)$definedName, \true);
                    ...
                    if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
                        ...
                        if (isset($mapSheetId[$activeTab]) && $mapSheetId[$activeTab] !== \null) {
        ...
        if (!$this->readDataOnly) {
            $contentTypes = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "[Content_Types].xml")), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
            foreach ($contentTypes->Override as $contentType) {
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->includeCharts) {
                            $chartEntryRef = \ltrim($contentType['PartName'], '/');
                            $chartElements = \simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, $chartEntryRef)), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
...
//        }
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->vertAlign) && isset($style->font->vertAlign["val"])) {
                ...
                if ($vertAlign == 'superscript') {
                    $docStyle->getFont()->setSuperScript(\true);
                }
                if ($vertAlign == 'subscript') {
                    $docStyle->getFont()->setSubScript(\true);
                }
            }
        }
        ...
        if (isset($style->fill)) {
            if ($style->fill->gradientFill) {
                ...
                if ($style->fill->patternFill->fgColor) {
                    $docStyle->getFill()->getStartColor()->setARGB(self::readColor($style->fill->patternFill->fgColor, \true));
                } else {
                    $docStyle->getFill()->getStartColor()->setARGB('FF000000');
                }
                if ($style->fill->patternFill->bgColor) {
                    $docStyle->getFill()->getEndColor()->setARGB(self::readColor($style->fill->patternFill->bgColor, \true));
                }
            }
        }
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if ((isset($run->rPr->b["val"]) && self::boolean((string) $run->rPr->b["val"])) ||
                            ...
                            $objText->getFont()->setBold(\true);
                        }
                        if ((isset($run->rPr->i["val"]) && self::boolean((string) $run->rPr->i["val"])) ||
                            (isset($run->rPr->i) && !isset($run->rPr->i["val"]))) {
                            $objText->getFont()->setItalic(\true);
                        }
                        if (isset($run->rPr->vertAlign) && isset($run->rPr->vertAlign["val"])) {
                            $vertAlign = \strtolower((string)$run->rPr->vertAlign["val"]);
                            if ($vertAlign == 'superscript') {
                                $objText->getFont()->setSuperScript(\true);
                            }
                            if ($vertAlign == 'subscript') {
                                $objText->getFont()->setSubScript(\true);
                            }
                        }
                        ...
                        if ((isset($run->rPr->strike["val"]) && self::boolean((string) $run->rPr->strike["val"])) ||
                            ...
                            $objText->getFont()->setStrikethrough(\true);
                        }
                    }
                }
            }
    ...
    private function readRibbon($excel, $customUITarget, $zip)
    {
        ...
        if ($dataRels) {
            ...
            $UIRels = \simplexml_load_string($this->securityScan($dataRels), 'SimpleXMLElement', \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
            if ($UIRels) {
        ...
        if ($localRibbon) {
            ...
            if (\count($customUIImagesNames) > 0 && \count($customUIImagesBinaries) > 0) {
                ...
                $excel->setRibbonBinObjects(\null);
            }
        } else {
            $excel->setRibbonXMLData(\null);
            $excel->setRibbonBinObjects(\null);
        }
    }
