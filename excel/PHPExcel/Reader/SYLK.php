    ...
    public function loadIntoExisting($pFilename, \PhpOffice\PhpSpreadsheet\Spreadsheet $objPHPExcel)
    {
        ...
        while (($rowData = \fgets($fileHandle)) !== \false) {
            ...
            //    Read cell value data
                ...
                // Set cell value
                $objPHPExcel->getActiveSheet()->getCell($columnLetter.$row, true)->setValue(($hasCalculatedValue) ? $cellDataFormula : $cellData);
                if ($hasCalculatedValue) {
                    $cellData = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::unwrapResult($cellData);
                    $objPHPExcel->getActiveSheet()->getCell($columnLetter.$row, true)->setCalculatedValue($cellData);
                }
            //    Read cell formatting
                ...
                if (($formatStyle > '') && ($column > '') && ($row > '')) {
                    ...
                    if (isset($this->formats[$formatStyle])) {
                        $objPHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($this->formats[$formatStyle], true);
                    }
                }
                if ((!empty($styleData)) && ($column > '') && ($row > '')) {
                    $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($column-1);
                    $objPHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($styleData, true);
                }
                if ($columnWidth > '') {
                    if ($startCol == $endCol) {
                        $startCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startCol-1);
                        $objPHPExcel->getActiveSheet()->getColumnDimension($startCol, true)->setWidth($columnWidth);
                    } else {
                        $startCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startCol-1);
