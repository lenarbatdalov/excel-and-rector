...
class StringHelper
{
    ...
    public static function UTF8toBIFF8UnicodeShort($value, $arrcRuns = array())
    {
        ...
        if (empty($arrcRuns)) {
            ...
            // characters
            $data .= self::ConvertEncoding($value, 'UTF-16LE', 'UTF-8');
            foreach ($arrcRuns as $arrcRun) {
                $data .= \pack('v', $arrcRun['strlen']);
                $data .= \pack('v', $arrcRun['fontidx']);
            }
        }
        return $data;
    }
     ...
     * @author vadik56
     */
    public static function utf16_decode($str, $bom_be = \true)
    {
        if (\strlen($str) < 2) {
            return $str;
        }
    ...
    public static function convertToNumberIfFraction(&$operand)
    {
        if (\preg_match('/^'.self::STRING_REGEXP_FRACTION.'$/i', $operand, $match)) {
            ...
            $operand = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::getInstance()->_calculateFormulaValue($fractionFormula);
            return \true;
        }
        return \false;
