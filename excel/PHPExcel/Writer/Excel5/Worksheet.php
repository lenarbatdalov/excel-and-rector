...
class Worksheet extends \PhpOffice\PhpSpreadsheet\Writer\Xls\BIFFwriter
{
     ...
     */
    private $phpExcelWriterExcel5Parser;
     ...
     * @param \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet
     */
...
//        $this->_limit = 8224;
        ...
        $this->colors            = &$colors;
        $this->phpExcelWriterExcel5Parser            = $parser;
        ...
        $highestRow  = $this->phpSheet->getHighestRow();
        $highestColumn = $this->phpSheet->getHighestColumn();

        // Determine lowest and highest column and row
//        $this->firstRowIndex = ($minR > 65535) ? 65535 : $minR;
        $this->lastRowIndex = ($highestRow > 65535) ? 65535 : $highestRow ;

        $this->firstColumnIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($minC);
        $this->lastColumnIndex  = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($highestColumn);
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
    ...
    public function close()
    {
        ...
        $sheetCount = $phpSheet->getParent()->getSheetCount();
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            if (isset($columnDimensions[$columnLetter])) {
                ...
                $hidden = $columnDimension->isVisible() ? 0 : 1;
        ...
        // Row dimensions
        foreach ($phpSheet->getRowDimensions() as $phpExcelWorksheetRowDimension) {
            $xfIndex = $phpExcelWorksheetRowDimension->getXfIndex() + 15; // there are 15 cellXfs
            $this->writeRow($phpExcelWorksheetRowDimension->getRowIndex() - 1, $phpExcelWorksheetRowDimension->getRowHeight(), $xfIndex, ($phpExcelWorksheetRowDimension->isVisible() ? '0' : '1'), $phpExcelWorksheetRowDimension->getOutlineLevel());
        }

        // Write Cells
        foreach ($phpSheet->getCoordinates() as $phpExcelCell) {
            $cell = $phpSheet->getCell($phpExcelCell, true);
            $row = $cell->getRow() - 1;
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
            ...
            $cVal = $cell->getValue();
            if ($cVal instanceof \PhpOffice\PhpSpreadsheet\RichText\RichText) {
                ...
                foreach ($elements as $element) {
                    $str_fontidx = $element instanceof \PhpOffice\PhpSpreadsheet\RichText\Run ? $this->fontHashIndex[$element->getFont()->getHashCode()] : 0;
        ...
        // Hyperlinks
        foreach ($phpSheet->getHyperLinkCollection() as $coordinate => $phpExcelCellHyperlink) {
            list($column, $row) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($coordinate);

            $url = $phpExcelCellHyperlink->getUrl();
            ...
            $this->writeUrl($row - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($column) - 1, $url);
        }
        ...
        if (!empty($arrConditionalStyles)) {
            ...
            foreach ($arrConditionalStyles as $arrConditionalStyle) {
                foreach ($arrConditionalStyle as $singleArrConditionalStyle) {
                    if (($singleArrConditionalStyle->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_EXPRESSION
                        || $singleArrConditionalStyle->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_CELLIS) && !\in_array($singleArrConditionalStyle->getHashCode(), $arrConditional)) {
                        $arrConditional[] = $singleArrConditionalStyle->getHashCode();
                        // Write CFRULE record
                        $this->writeCFRule($singleArrConditionalStyle);
                    }
                }
            }
        }
    ...
    private function writeBIFF8CellRangeAddressFixed($range = 'A1')
    {
        ...
        $firstCellCoordinates = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($firstCell); // e.g. array(0, 1)
        $lastCellCoordinates  = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($lastCell);  // e.g. array(1, 6)

        return \pack('vvvv', $firstCellCoordinates[1] - 1, $lastCellCoordinates[1] - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($firstCellCoordinates[0]) - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($lastCellCoordinates[0]) - 1);
    }
     ...
     */
    public function setOutline($visible = \true, $symbols_below = \true, $symbols_right = \true, $auto_style = \false)
    {
    ...
    private function writeNumber($row, $col, $num, $xfIndex)
    {
        ...
        if (self::getByteOrder() !== 0) { // if it's Big Endian
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
    {
        ...
        try {
            $error = $this->phpExcelWriterExcel5Parser->parse($formula);
            $formula = $this->phpExcelWriterExcel5Parser->toReversePolish();
    ...
    private function writeWindow2()
    {
        ...
        $zoomScaleNormal =  $this->phpSheet->getSheetView()->getZoomScaleNormal();

        $data .= \pack("vvvvV", $rgbHdr, 0x0000, $zoom_factor_page_break, $zoomScaleNormal, 0x00000000);
    ...
    private function writeSelection()
    {
        ...
        $selectedCells = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::splitRange($this->phpSheet->getSelectedCells());
        ...
        list($colFirst, $rwFirst) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($first);
        $colFirst = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($colFirst) - 1; // base 0 column index
        --$rwFirst; // base 0 row index

        list($colLast, $rwLast) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($last);
        $colLast = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($colLast) - 1; // base 0 column index
    ...
    private function writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            $range = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::splitRange($mergeCell);
            list($first, $last) = $range[0];
            list($firstColumn, $firstRow) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($first);
            list($lastColumn, $lastRow) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($last);

            $recordData .= \pack('vvvv', $firstRow - 1, $lastRow - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($firstColumn) - 1, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($lastColumn) - 1);
    ...
    private function writePanes()
    {
        ...
        if (($freezePane = $this->phpSheet->getFreezePane()) !== '') {
            list($column, $row) = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($freezePane);
            $panes[0] = $row - 1;
            $panes[1] = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($column) - 1;
        } else {
            // thaw panes
            return;
        }

        $y       = isset($panes[0]) ? $panes[0] : \null;
        $x       = isset($panes[1]) ? $panes[1] : \null;
        $rwTop   = isset($panes[2]) ? $panes[2] : \null;
        $colLeft = isset($panes[3]) ? $panes[3] : \null;
        $pnnAct = \count($panes) > 4 ? $panes[4] : \null;
    ...
    private function writeSetup()
    {
        ...
        $iScale = $this->phpSheet->getPageSetup()->getScale() !== 0 ?
            $this->phpSheet->getPageSetup()->getScale() : 100;   // Print scaling factor

        $iPageStart   = 0x01;                 // Starting page number
        $iFitWidth    = $this->phpSheet->getPageSetup()->getFitToWidth();    // Fit to number of pages wide
        $iFitHeight    = $this->phpSheet->getPageSetup()->getFitToHeight();    // Fit to number of pages high
    ...
    private function writeMarginLeft()
    {
        ...
        $left  = $this->phpSheet->getPageMargins()->getLeft();     // Margin in inches
    ...
    private function writeMarginRight()
    {
        ...
        $right  = $this->phpSheet->getPageMargins()->getRight();     // Margin in inches
    ...
    private function writeMarginTop()
    {
        ...
        $top  = $this->phpSheet->getPageMargins()->getTop();     // Margin in inches
    ...
    private function writeMarginBottom()
    {
        ...
        $bottom  = $this->phpSheet->getPageMargins()->getBottom();     // Margin in inches
    ...
    private function writeAutoFilterInfo()
    {
        ...
        $rangeBounds = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries($this->phpSheet->getAutoFilter()->getRange());
    ...
    private function writeGuts()
    {
        ...
        foreach ($this->phpSheet->getRowDimensions() as $phpExcelWorksheetRowDimension) {
            $maxRowOutlineLevel = \max($maxRowOutlineLevel, $phpExcelWorksheetRowDimension->getOutlineLevel());
        }
        ...
        foreach ($this->phpSheet->getBreaks() as $cell => $breakType) {
            ...
            switch ($breakType) {
                case \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::BREAK_COLUMN:
                    ...
                    $vbreaks[] = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($coordinates[0]) - 1;
                    break;
    ...
    public function positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        if ($x1 >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start))) {
            $x1 = 0;
        }
        ...
        while ($width >= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))) {
            $width -= \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end));
            ++$col_end;
        }
        ...
        //
        if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start)) == 0) {
            return;
        }
        if (\PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))   == 0) {
            return;
        }
        ...
        $x1 = $x1     / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_start))   * 1024;
        $y1 = $y1     / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeRow($this->phpSheet, $row_start + 1)   *  256;
        $x2 = $width  / \PhpOffice\PhpSpreadsheet\Shared\Xls::sizeCol($this->phpSheet, \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col_end))     * 1024; // Distance to right side of object
    ...
    {
        return $this->phpExcelSharedEscher;
    }

    /**
     * Set Escher object
     */
    public function setEscher(\PhpOffice\PhpSpreadsheet\Shared\Escher $phpExcelSharedEscher = \null)
    {
        $this->phpExcelSharedEscher = $phpExcelSharedEscher;
    }
    ...
    private function writeMsoDrawing()
    {
        ...
        if (property_exists($this, 'phpExcelSharedEscher') && $this->phpExcelSharedEscher !== null) {
            $phpExcelWriterExcel5Escher = new \PhpOffice\PhpSpreadsheet\Writer\Xls\Escher($this->phpExcelSharedEscher);
            $data = $phpExcelWriterExcel5Escher->close();
            $spOffsets = $phpExcelWriterExcel5Escher->getSpOffsets();
            $spTypes = $phpExcelWriterExcel5Escher->getSpTypes();
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                // empty cells allowed
                $options |= $dataValidation->isAllowBlank() << 8;

                // show drop down
                $options |= (!$dataValidation->isShowDropDown()) << 9;

                // show input message
                $options |= $dataValidation->isShowInputMessage() << 18;

                // show error message
                $options |= $dataValidation->isShowErrorMessage() << 19;
                ...
                try {
                    ...
                    $this->phpExcelWriterExcel5Parser->parse($formula1);
                    $formula1 = $this->phpExcelWriterExcel5Parser->toReversePolish();
                    $sz1 = \strlen($formula1);
                } catch (\PhpOffice\PhpSpreadsheet\Exception $e) {
                    $sz1 = 0;
                    $formula1 = '';
                ...
                try {
                    ...
                    $this->phpExcelWriterExcel5Parser->parse($formula2);
                    $formula2 = $this->phpExcelWriterExcel5Parser->toReversePolish();
                    $sz2 = \strlen($formula2);
                } catch (\PhpOffice\PhpSpreadsheet\Exception $e) {
                    $sz2 = 0;
                    $formula2 = '';
     ...
     * Write CFRule Record
     */
    private function writeCFRule(\PhpOffice\PhpSpreadsheet\Style\Conditional $phpExcelStyleConditional)
    {
        $record      = 0x01B1;               // Record identifier

        // $type : Type of the CF
        // $operatorType : Comparison operator
        if ($phpExcelStyleConditional->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_EXPRESSION) {
            $type = 0x02;
            $operatorType = 0x00;
        } elseif ($phpExcelStyleConditional->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_CELLIS) {
            $type = 0x01;

            switch ($phpExcelStyleConditional->getOperatorType()) {
        ...
        $arrConditions = $phpExcelStyleConditional->getConditions();
        ...
        if ($numConditions == 1) {
            ...
            $operand2 = \null;
        } elseif ($numConditions == 2 && ($phpExcelStyleConditional->getOperatorType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::OPERATOR_BETWEEN)) {
            $szValue1 = ($arrConditions[0] <= 65535 ? 3 : 0x0000);
        ...
        // $flags : Option flags
        // Alignment
        $bAlignHz = ($phpExcelStyleConditional->getStyle()->getAlignment()->getHorizontal() == \null ? 1 : 0);
        $bAlignVt = ($phpExcelStyleConditional->getStyle()->getAlignment()->getVertical() == \null ? 1 : 0);
        $bAlignWrapTx = ($phpExcelStyleConditional->getStyle()->getAlignment()->getWrapText() == \false ? 1 : 0);
        $bTxRotation = ($phpExcelStyleConditional->getStyle()->getAlignment()->getTextRotation() == \null ? 1 : 0);
        $bIndent = ($phpExcelStyleConditional->getStyle()->getAlignment()->getIndent() == 0 ? 1 : 0);
        $bShrinkToFit = ($phpExcelStyleConditional->getStyle()->getAlignment()->getShrinkToFit() == \false ? 1 : 0);
        ...
        // Protection
        $bProtLocked = ($phpExcelStyleConditional->getStyle()->getProtection()->getLocked() == \null ? 1 : 0);
        $bProtHidden = ($phpExcelStyleConditional->getStyle()->getProtection()->getHidden() == \null ? 1 : 0);
        $bFormatProt = $bProtLocked == 0 || $bProtHidden == 0 ? 1 : 0;
        // Border
        $bBorderLeft = ($phpExcelStyleConditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB() == \PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK
                        && $phpExcelStyleConditional->getStyle()->getBorders()->getLeft()->getBorderStyle() == \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderRight = ($phpExcelStyleConditional->getStyle()->getBorders()->getRight()->getColor()->getARGB() == \PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK
                        && $phpExcelStyleConditional->getStyle()->getBorders()->getRight()->getBorderStyle() == \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderTop = ($phpExcelStyleConditional->getStyle()->getBorders()->getTop()->getColor()->getARGB() == \PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK
                        && $phpExcelStyleConditional->getStyle()->getBorders()->getTop()->getBorderStyle() == \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE ? 1 : 0);
        $bBorderBottom = ($phpExcelStyleConditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB() == \PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK
                        && $phpExcelStyleConditional->getStyle()->getBorders()->getBottom()->getBorderStyle() == \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE ? 1 : 0);
        $bFormatBorder = $bBorderLeft == 0 || $bBorderRight == 0 || $bBorderTop == 0 || $bBorderBottom == 0 ? 1 : 0;
        // Pattern
        $bFillStyle = ($phpExcelStyleConditional->getStyle()->getFill()->getFillType() == \null ? 0 : 1);
        $bFillColor = ($phpExcelStyleConditional->getStyle()->getFill()->getStartColor()->getARGB() == \null ? 0 : 1);
        $bFillColorBg = ($phpExcelStyleConditional->getStyle()->getFill()->getEndColor()->getARGB() == \null ? 0 : 1);
        $bFormatFill = $bFillStyle == 0 || $bFillColor == 0 || $bFillColorBg == 0 ? 1 : 0;
        // Font
        if ($phpExcelStyleConditional->getStyle()->getFont()->getName() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getSize() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getBold() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getItalic() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getSuperScript() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getSubScript() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getUnderline() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getStrikethrough() != \null
            || $phpExcelStyleConditional->getStyle()->getFont()->getColor()->getARGB() != \null) {
            $bFormatFont = 1;
        } else {
            $bFormatFont = 0;
        }
        ...
        if ($bFormatFont == 1) {
            ...
            if ($phpExcelStyleConditional->getStyle()->getFont()->getName() == \null) {
                ...
                $dataBlockFont = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::UTF8toBIFF8UnicodeLong($phpExcelStyleConditional->getStyle()->getFont()->getName());
            }
            // Font Size
            if ($phpExcelStyleConditional->getStyle()->getFont()->getSize() == \null) {
                $dataBlockFont .= \pack('V', 20 * 11);
            } else {
                $dataBlockFont .= \pack('V', 20 * $phpExcelStyleConditional->getStyle()->getFont()->getSize());
            }
            // Font Options
            $dataBlockFont .= \pack('V', 0);
            // Font weight
            if ($phpExcelStyleConditional->getStyle()->getFont()->getBold() == \true) {
                $dataBlockFont .= \pack('v', 0x02BC);
            } else {
                $dataBlockFont .= \pack('v', 0x0190);
            }
            // Escapement type
            if ($phpExcelStyleConditional->getStyle()->getFont()->getSubScript() == \true) {
                $dataBlockFont .= \pack('v', 0x02);
                $fontEscapement = 0;
            } elseif ($phpExcelStyleConditional->getStyle()->getFont()->getSuperScript() == \true) {
            ...
            // Underline type
            switch ($phpExcelStyleConditional->getStyle()->getFont()->getUnderline()) {
            ...
            // Font color index
            switch ($phpExcelStyleConditional->getStyle()->getFont()->getColor()->getRGB()) {
                case '000000':
                    $colorIdx = 0x08;
                    break;
                case 'FFFFFF':
            ...
            $optionsFlags = 0;
            $optionsFlagsBold = ($phpExcelStyleConditional->getStyle()->getFont()->getBold() == \null ? 1 : 0);
        ...
        if ($bFormatAlign == 1) {
            ...
            switch ($phpExcelStyleConditional->getStyle()->getAlignment()->getHorizontal()) {
            ...
            if ($phpExcelStyleConditional->getStyle()->getAlignment()->getWrapText() == \true) {
                $blockAlign |= 1 << 3;
            } else {
                $blockAlign |= 0 << 3;
            }
            switch ($phpExcelStyleConditional->getStyle()->getAlignment()->getVertical()) {
            ...
            // Text rotation angle
            $blockRotation = $phpExcelStyleConditional->getStyle()->getAlignment()->getTextRotation();

            // Indentation
            $blockIndent = $phpExcelStyleConditional->getStyle()->getAlignment()->getIndent();
            if ($phpExcelStyleConditional->getStyle()->getAlignment()->getShrinkToFit() == \true) {
                $blockIndent |= 1 << 4;
            } else {
                $blockIndent |= 0 << 4;
            }
        ...
        if ($bFormatBorder == 1) {
            $blockLineStyle = 0;
            switch ($phpExcelStyleConditional->getStyle()->getBorders()->getLeft()->getBorderStyle()) {
            ...
            switch ($phpExcelStyleConditional->getStyle()->getBorders()->getRight()->getBorderStyle()) {
                case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE:
            ...
            switch ($phpExcelStyleConditional->getStyle()->getBorders()->getTop()->getBorderStyle()) {
                case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE:
            ...
            switch ($phpExcelStyleConditional->getStyle()->getBorders()->getBottom()->getBorderStyle()) {
                case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE:
            ...
            switch ($phpExcelStyleConditional->getStyle()->getBorders()->getDiagonal()->getBorderStyle()) {
                case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE:
                    $blockColor |= 0x00 << 21;
                    break;
                case \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN:
        ...
        if ($bFormatFill == 1) {
            ...
            switch ($phpExcelStyleConditional->getStyle()->getFill()->getFillType()) {
                ...
                case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_PATTERN_GRAY0625:
                    ...
                    break;
                case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_GRADIENT_LINEAR:    // does not exist in BIFF8
                case \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_GRADIENT_PATH:    // does not exist in BIFF8
                default:
                    $blockFillPatternStyle = 0x00;
                    break;
            }
            // Color
            switch ($phpExcelStyleConditional->getStyle()->getFill()->getStartColor()->getRGB()) {
                case '000000':
                    $colorIdxBg = 0x08;
                    break;
                case 'FFFFFF':
            ...
            // Fg Color
            switch ($phpExcelStyleConditional->getStyle()->getFill()->getEndColor()->getRGB()) {
                case '000000':
                    $colorIdxFg = 0x08;
                    break;
                case 'FFFFFF':
        ...
        if ($bFormatProt == 1) {
            ...
            if ($phpExcelStyleConditional->getStyle()->getProtection()->getLocked() == \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_PROTECTED) {
                $dataBlockProtection = 1;
            }
            if ($phpExcelStyleConditional->getStyle()->getProtection()->getHidden() == \PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_PROTECTED) {
    ...
    private function writeCFHeader()
    {
        ...
        foreach ($this->phpSheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            foreach ($conditionalStyles as $conditionalStyle) {
                if ($conditionalStyle->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_EXPRESSION
                        || $conditionalStyle->getConditionType() == \PhpOffice\PhpSpreadsheet\Style\Conditional::CONDITION_CELLIS) {
                    if (!\in_array($conditionalStyle->getHashCode(), $arrConditional)) {
                        $arrConditional[] = $conditionalStyle->getHashCode();
                    }
                    // Cells
                    $arrCoord = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateFromString($cellCoordinate);
                    if (!\is_numeric($arrCoord[0])) {
                        $arrCoord[0] = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($arrCoord[0]);
                    }
