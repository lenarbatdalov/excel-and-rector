...
//        $this->firstRowIndex = ($minR > 65535) ? 65535 : $minR;
        ...
        $this->lastColumnIndex  = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($maxC);
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
    ...
    public function close()
    {
        ...
        // Write Cells
        foreach ($phpSheet->getCoordinates() as $cellID) {
            $cell = $phpSheet->getCell($cellID, true);
            $row = $cell->getRow() - 1;
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
     ...
     */
    public function setEscher(\PhpOffice\PhpSpreadsheet\Shared\Escher $pValue = \null)
    {
        $this->escher = $pValue;
    }
    ...
    private function writeCFRule(\PhpOffice\PhpSpreadsheet\Style\Conditional $conditional)
    {
        ...
        // $flags : Option flags
        // Alignment
        $bAlignHz = ($conditional->getStyle()->getAlignment()->getHorizontal() == \null ? 1 : 0);
        $bAlignVt = ($conditional->getStyle()->getAlignment()->getVertical() == \null ? 1 : 0);
        $bAlignWrapTx = ($conditional->getStyle()->getAlignment()->getWrapText() == \false ? 1 : 0);
        $bTxRotation = ($conditional->getStyle()->getAlignment()->getTextRotation() == \null ? 1 : 0);
        ...
        // Protection
        $bProtLocked = ($conditional->getStyle()->getProtection()->getLocked() == \null ? 1 : 0);
        $bProtHidden = ($conditional->getStyle()->getProtection()->getHidden() == \null ? 1 : 0);
        ...
        // Pattern
        $bFillStyle = ($conditional->getStyle()->getFill()->getFillType() == \null ? 0 : 1);
        $bFillColor = ($conditional->getStyle()->getFill()->getStartColor()->getARGB() == \null ? 0 : 1);
        $bFillColorBg = ($conditional->getStyle()->getFill()->getEndColor()->getARGB() == \null ? 0 : 1);
        ...
        if ($conditional->getStyle()->getFont()->getName() != \null
            ...
            || $conditional->getStyle()->getFont()->getSuperScript() != \null
            || $conditional->getStyle()->getFont()->getSubScript() != \null
            || $conditional->getStyle()->getFont()->getUnderline() != \null
            || $conditional->getStyle()->getFont()->getStrikethrough() != \null
        ...
        if ($bFormatFont == 1) {
            ...
            $optionsFlags = 0;
            $optionsFlagsBold = ($conditional->getStyle()->getFont()->getBold() == \null ? 1 : 0);
