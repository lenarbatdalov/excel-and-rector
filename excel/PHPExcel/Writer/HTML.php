 ...
 */
class Html extends \PhpOffice\PhpSpreadsheet\Writer\BaseWriter implements \PhpOffice\PhpSpreadsheet\Writer\IWriter
{
    /**
     * PHPExcel object
     *
     ...
     * @var boolean
     */
    private $generateSheetNavigationBlock = \true;
    ...
    public function __construct(\PhpOffice\PhpSpreadsheet\Spreadsheet $phpExcel)
    {
        ...
        $this->phpExcelStyleFont = $this->phpExcel->getDefaultStyle()->getFont();
    }
     ...
     * @return boolean
     */
    public function isGenerateSheetNavigationBlock()
    {
    ...
    {
        $this->generateSheetNavigationBlock = $pValue;
        return $this;
    }
    ...
    public function generateHTMLHeader($pIncludeStyles = \false)
    {
        ...
        if (\is_null($this->phpExcel)) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\Exception('Internal PHPExcel object not set to an instance of an object.');
        }

        // Construct HTML
        $phpExcelDocumentProperties = $this->phpExcel->getProperties();
        ...
        if ($phpExcelDocumentProperties->getTitle() > '') {
            $html .= '      <title>' . \htmlspecialchars($phpExcelDocumentProperties->getTitle()) . '</title>' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getCreator() > '') {
            $html .= '      <meta name="author" content="' . \htmlspecialchars($phpExcelDocumentProperties->getCreator()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getTitle() > '') {
            $html .= '      <meta name="title" content="' . \htmlspecialchars($phpExcelDocumentProperties->getTitle()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getDescription() > '') {
            $html .= '      <meta name="description" content="' . \htmlspecialchars($phpExcelDocumentProperties->getDescription()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getSubject() > '') {
            $html .= '      <meta name="subject" content="' . \htmlspecialchars($phpExcelDocumentProperties->getSubject()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getKeywords() > '') {
            $html .= '      <meta name="keywords" content="' . \htmlspecialchars($phpExcelDocumentProperties->getKeywords()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getCategory() > '') {
            $html .= '      <meta name="category" content="' . \htmlspecialchars($phpExcelDocumentProperties->getCategory()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getCompany() > '') {
            $html .= '      <meta name="company" content="' . \htmlspecialchars($phpExcelDocumentProperties->getCompany()) . '" />' . \PHP_EOL;
        }
        if ($phpExcelDocumentProperties->getManager() > '') {
            $html .= '      <meta name="manager" content="' . \htmlspecialchars($phpExcelDocumentProperties->getManager()) . '" />' . \PHP_EOL;
        }
            ...
            while ($row++ < $rowMax) {
                ...
                if (!isset($this->isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    ...
                    while ($column++ < $dimension[1][0]) {
                        // Cell exists?
                        if ($sheet->cellExistsByColumnAndRow($column + 1, $row)) {
    ...
    private function extendRowsForChartsAndImages(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpExcelWorksheet, $row)
    {
        $rowMax = $row;
        $colMax = 'A';
        if ($this->includeCharts) {
            foreach ($phpExcelWorksheet->getChartCollection() as $chart) {
        ...
        foreach ($phpExcelWorksheet->getDrawingCollection() as $drawing) {
        ...
        while ($row <= $rowMax) {
            ...
            for ($col = 'A'; $col != $colMax; ++$col) {
                $html .= '<td>';
                $html .= $this->writeImageInCell($phpExcelWorksheet, $col.$row);
                if ($this->includeCharts) {
                    $html .= $this->writeChartInCell($phpExcelWorksheet, $col.$row);
                }
                $html .= '</td>';
            }
            ++$row;
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet    $phpExcelWorksheet            PHPExcel_Worksheet
     * @param    string                $coordinates    Cell coordinates
     * @return    string
     * @throws    \PhpOffice\PhpSpreadsheet\Writer\Exception
     */
    private function writeImageInCell(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpExcelWorksheet, $coordinates)
    {
        // Construct HTML
        $html = '';
     ...
     * @throws    \PhpOffice\PhpSpreadsheet\Writer\Exception
     */
    private function writeChartInCell(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpExcelWorksheet, $coordinates)
    {
        // Construct HTML
        $html = '';
    ...
    public function buildCSS($generateSurroundingHTML = \true)
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width = \PhpOffice\PhpSpreadsheet\Shared\Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->phpExcelStyleFont)) >= 0) {
            ...
            if ($rowDimension->getRowHeight() == -1) {
                $pt_height = \PhpOffice\PhpSpreadsheet\Shared\Font::getDefaultRowHeightByFont($this->phpExcel->getDefaultStyle()->getFont());
            } else {
     ...
     * Create CSS style
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Style        $phpExcelStyle            PHPExcel_Style
     * @return    array
     */
    private function createCSSStyle(\PhpOffice\PhpSpreadsheet\Style\Style $phpExcelStyle)
    {
        // Create CSS
        $css = \array_merge(
            $this->createCSSStyleAlignment($phpExcelStyle->getAlignment()),
            $this->createCSSStyleBorders($phpExcelStyle->getBorders()),
            $this->createCSSStyleFont($phpExcelStyle->getFont()),
            $this->createCSSStyleFill($phpExcelStyle->getFill())
        );
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Alignment        $phpExcelStyleAlignment            PHPExcel_Style_Alignment
     * @return    array
     */
    private function createCSSStyleAlignment(\PhpOffice\PhpSpreadsheet\Style\Alignment $phpExcelStyleAlignment)
    {
        // Construct CSS
        $css = array();

        // Create CSS
        $css['vertical-align'] = $this->mapVAlign($phpExcelStyleAlignment->getVertical());
        if ($textAlign = $this->mapHAlign($phpExcelStyleAlignment->getHorizontal())) {
            $css['text-align'] = $textAlign;
            if (\in_array($textAlign, array('left', 'right'))) {
                $css['padding-'.$textAlign] = (int)$phpExcelStyleAlignment->getIndent() * 9.'px';
            }
        }
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Font        $phpExcelStyleFont            PHPExcel_Style_Font
     * @return    array
     */
    private function createCSSStyleFont(\PhpOffice\PhpSpreadsheet\Style\Font $phpExcelStyleFont)
    {
        // Construct CSS
        $css = array();
        ...
        if ($phpExcelStyleFont->getUnderline() != \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE && $phpExcelStyleFont->getStrikethrough()) {
            $css['text-decoration'] = 'underline line-through';
        } elseif ($phpExcelStyleFont->getUnderline() != \PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE) {
            $css['text-decoration'] = 'underline';
        } elseif ($phpExcelStyleFont->getStrikethrough()) {
        ...
        $css['color']       = '#' . $phpExcelStyleFont->getColor()->getRGB();
        $css['font-family'] = '\'' . $phpExcelStyleFont->getName() . '\'';
        $css['font-size']   = $phpExcelStyleFont->getSize() . 'pt';
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Borders        $phpExcelStyleBorders            PHPExcel_Style_Borders
     * @return    array
     */
    private function createCSSStyleBorders(\PhpOffice\PhpSpreadsheet\Style\Borders $phpExcelStyleBorders)
    {
        // Construct CSS
        $css = array();

        // Create CSS
        $css['border-bottom'] = $this->createCSSStyleBorder($phpExcelStyleBorders->getBottom());
        $css['border-top']    = $this->createCSSStyleBorder($phpExcelStyleBorders->getTop());
        $css['border-left']   = $this->createCSSStyleBorder($phpExcelStyleBorders->getLeft());
        $css['border-right']  = $this->createCSSStyleBorder($phpExcelStyleBorders->getRight());
     ...
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Border        $phpExcelStyleBorder            PHPExcel_Style_Border
     * @return    string
     */
    private function createCSSStyleBorder(\PhpOffice\PhpSpreadsheet\Style\Border $phpExcelStyleBorder)
    {
        // Create CSS
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
        //    Create CSS - add !important to non-none border styles for merged cells
        $borderStyle = $this->mapBorderStyle($phpExcelStyleBorder->getBorderStyle());

        return $borderStyle . ' #' . $phpExcelStyleBorder->getColor()->getRGB() . (($borderStyle == 'none') ? '' : ' !important');
    }

    /**
     * Create CSS style (PHPExcel_Style_Fill)
     *
     * @param \PhpOffice\PhpSpreadsheet\Style\Fill        $phpExcelStyleFill            PHPExcel_Style_Fill
     * @return    array
     */
    private function createCSSStyleFill(\PhpOffice\PhpSpreadsheet\Style\Fill $phpExcelStyleFill)
    {
        // Construct HTML
        $css = array();

        // Create CSS
        $value = $phpExcelStyleFill->getFillType() == \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_NONE ?
            'white' : '#' . $phpExcelStyleFill->getStartColor()->getRGB();
     ...
     * @throws    \PhpOffice\PhpSpreadsheet\Writer\Exception
     */
    private function generateRow(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpExcelWorksheet, $pValues = \null, $pRow = 0, $cellType = 'td')
    {
        if (\is_array($pValues)) {
            // Construct HTML
            $html = '';
            ...
            // Sheet index
            $sheetIndex = $phpExcelWorksheet->getParent()->getIndex($phpExcelWorksheet);

            // DomPDF and breaks
            if ($this->isPdf && \count($phpExcelWorksheet->getBreaks()) > 0) {
                $breaks = $phpExcelWorksheet->getBreaks();
                ...
                if (isset($breaks['A' . $pRow])) {
                    ...
                    $html .= $this->generateTableHeader($phpExcelWorksheet);
                }
            }
            ...
            foreach ($pValues as $cellAddress) {
                $cell = ($cellAddress > '') ? $phpExcelWorksheet->getCell($cellAddress) : '';
                ...
                if ($cell instanceof \PhpOffice\PhpSpreadsheet\Cell\Cell) {
                    ...
                    if ($cell->getValue() instanceof \PhpOffice\PhpSpreadsheet\RichText\RichText) {
                        ...
                        if ($this->preCalculateFormulas) {
                            $cellData = \PhpOffice\PhpSpreadsheet\Style\NumberFormat::toFormattedString(
                                ...
                                $phpExcelWorksheet->getParent()->getCellXfByIndex($cell->getXfIndex())->getNumberFormat()->getFormatCode(),
                        ...
                        if ($phpExcelWorksheet->getParent()->getCellXfByIndex($cell->getXfIndex())->getFont()->getSuperScript()) {
                            $cellData = '<sup>'.$cellData.'</sup>';
                        } elseif ($phpExcelWorksheet->getParent()->getCellXfByIndex($cell->getXfIndex())->getFont()->getSubScript()) {
                    ...
                    if (!$this->useInlineCss) {
                        ...
                        $sharedStyle = $phpExcelWorksheet->getParent()->getCellXfByIndex($cell->getXfIndex());
                ...
                // Hyperlink?
                if ($phpExcelWorksheet->hyperlinkExists($coordinate) && !$phpExcelWorksheet->getHyperlink($coordinate)->isInternal()) {
                    $cellData = '<a href="' . \htmlspecialchars($phpExcelWorksheet->getHyperlink($coordinate)->getUrl()) . '" title="' . \htmlspecialchars($phpExcelWorksheet->getHyperlink($coordinate)->getTooltip()) . '">' . $cellData . '</a>';
                }

                // Should the cell be written or is it swallowed by a rowspan or colspan?
                $writeCell = !(isset($this->isSpannedCell[$phpExcelWorksheet->getParent()->getIndex($phpExcelWorksheet)][$pRow + 1][$colNum])
                            && $this->isSpannedCell[$phpExcelWorksheet->getParent()->getIndex($phpExcelWorksheet)][$pRow + 1][$colNum]);

                // Colspan and Rowspan
                $colspan = 1;
                $rowspan = 1;
                if (isset($this->isBaseCell[$phpExcelWorksheet->getParent()->getIndex($phpExcelWorksheet)][$pRow + 1][$colNum])) {
                    $spans = $this->isBaseCell[$phpExcelWorksheet->getParent()->getIndex($phpExcelWorksheet)][$pRow + 1][$colNum];
                    ...
                    if (!$this->useInlineCss) {
                        $cssClass .= ' style' . $phpExcelWorksheet->getCell($endCellCoord)->getXfIndex();
                    }
                }
                ...
                if ($writeCell) {
                    ...
                    // Image?
                    $html .= $this->writeImageInCell($phpExcelWorksheet, $coordinate);

                    // Chart?
                    if ($this->includeCharts) {
                        $html .= $this->writeChartInCell($phpExcelWorksheet, $coordinate);
                    }
     ...
     * @return boolean
     */
    public function isUseInlineCss()
    {
    ...
    private function calculateSpans()
    {
        ...
        foreach ($sheetIndexes as $sheetIndex) {
            ...
            foreach ($candidateSpannedRow as $singleCandidateSpannedRow) {
                if (isset($this->isSpannedCell[$sheetIndex][$singleCandidateSpannedRow]) && \count($this->isSpannedCell[$sheetIndex][$singleCandidateSpannedRow]) == $countColumns) {
                    $this->isSpannedRow[$sheetIndex][$singleCandidateSpannedRow] = $singleCandidateSpannedRow;;
                }
            }
        ...
        $this->spansAreCalculated = \true;
    }

    private function setMargins(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpExcelWorksheet)
    {
        $htmlPage = '@page { ';
        $htmlBody = 'body { ';

        $left = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::FormatNumber($phpExcelWorksheet->getPageMargins()->getLeft()) . 'in; ';
        $htmlPage .= 'margin-left: ' . $left;
        $htmlBody .= 'margin-left: ' . $left;
        $right = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::FormatNumber($phpExcelWorksheet->getPageMargins()->getRight()) . 'in; ';
        $htmlPage .= 'margin-right: ' . $right;
        $htmlBody .= 'margin-right: ' . $right;
        $top = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::FormatNumber($phpExcelWorksheet->getPageMargins()->getTop()) . 'in; ';
        $htmlPage .= 'margin-top: ' . $top;
        $htmlBody .= 'margin-top: ' . $top;
        $bottom = \PhpOffice\PhpSpreadsheet\Shared\StringHelper::FormatNumber($phpExcelWorksheet->getPageMargins()->getBottom()) . 'in; ';
