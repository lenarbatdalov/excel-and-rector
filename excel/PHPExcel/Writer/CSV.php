...
class Csv extends \PhpOffice\PhpSpreadsheet\Writer\BaseWriter implements \PhpOffice\PhpSpreadsheet\Writer\IWriter
{
    ...
    {
        // Fetch sheet
        $phpExcelWorksheet = $this->phpExcel->getSheet($this->sheetIndex);
        ...
        $maxCol = $phpExcelWorksheet->getHighestDataColumn();
        $maxRow = $phpExcelWorksheet->getHighestDataRow();

        // Write rows to file
        for ($row = 1; $row <= $maxRow; ++$row) {
            // Convert the row to an array...
            $cellsArray = $phpExcelWorksheet->rangeToArray('A'.$row.':'.$maxCol.$row, '', $this->preCalculateFormulas);
     ...
     * @return boolean
     */
    public function isIncludeSeparatorLine()
    {
     ...
     * @return boolean
     */
    public function isExcelCompatibility()
    {
    ...
    private function writeLine($pFileHandle = \null, $pValues = \null)
    {
        if (\is_array($pValues)) {
            ...
            foreach ($pValues as $pValue) {
                // Escape enclosures
                $pValue = \str_replace($this->enclosure, $this->enclosure . $this->enclosure, $pValue);
                ...
                // Add enclosed string
                $line .= $this->enclosure . $pValue . $this->enclosure;
            }
