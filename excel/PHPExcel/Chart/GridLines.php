 ...
 * Time: 2:36 PM
 */
class GridLines extends \PHPExcel_Chart_Properties
{
   ...
   */
    ...
    public function setLineStyleProperties($line_width = \null, $compound_type = \null, $dash_type = \null, $cap_type = \null, $join_type = \null, $head_arrow_type = \null, $head_arrow_size = \null, $end_arrow_type = \null, $end_arrow_size = \null)
    {
        ...
        (\is_null($line_width))
                ? \null
                : ($this->lineProperties['style']['width'] = $this->getExcelPointsWidth($line_width));
        (\is_null($compound_type))
                ? \null
                : ($this->lineProperties['style']['compound'] = $compound_type);
        (\is_null($dash_type))
                ? \null
                : ($this->lineProperties['style']['dash'] = $dash_type);
        (\is_null($cap_type))
                ? \null
                : ($this->lineProperties['style']['cap'] = $cap_type);
        (\is_null($join_type))
                ? \null
                : ($this->lineProperties['style']['join'] = $join_type);
        (\is_null($head_arrow_type))
                ? \null
                : ($this->lineProperties['style']['arrow']['head']['type'] = $head_arrow_type);
        (\is_null($head_arrow_size))
                ? \null
                : ($this->lineProperties['style']['arrow']['head']['size'] = $head_arrow_size);
        (\is_null($end_arrow_type))
                ? \null
                : ($this->lineProperties['style']['arrow']['end']['type'] = $end_arrow_type);
        (\is_null($end_arrow_size))
                ? \null
                : ($this->lineProperties['style']['arrow']['end']['size'] = $end_arrow_size);
    }
    ...
    {
        if (!\is_null($color)) {
            $this->glowProperties['color']['value'] = $color;
        }
        if (!\is_null($alpha)) {
            $this->glowProperties['color']['alpha'] = $this->getTrueAlpha($alpha);
        }
        if (!\is_null($type)) {
            $this->glowProperties['color']['type'] = $type;
        }
    ...
    {
        $this->activateObject()
            ->setShadowPresetsProperties($sh_presets)
            ->setShadowColor(
    ...
    {
        if (!\is_null($color)) {
            $this->shadowProperties['color']['value'] = $color;
        }
        if (!\is_null($alpha)) {
            $this->shadowProperties['color']['alpha'] = $this->getTrueAlpha($alpha);
        }
        if (!\is_null($type)) {
            $this->shadowProperties['color']['type'] = $type;
        }
